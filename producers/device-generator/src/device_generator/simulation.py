...
def set_telemetry_sink(sink: "queue.Queue[dict[str, Any]] | None") -> None:
    """Register a queue to receive a copy of each telemetry record (for SSE etc.). Thread-safe."""
...
def _one_record(patient_id: str, ts_ms: int, software_version: str = "1.2.0") -> list[dict[str, Any]]:
...
def start_simulation(
    *,
    ...
    Args:
        patient_id: When simulation_type is "single", simulate only this patient.
        simulation_type: "all" = N patients from config; "single" = only patient_id.
    """
